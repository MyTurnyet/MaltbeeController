...
### Not yet wired: BootMode/setup-mode trigger classes (sub-project #2c-a) and the wireless commissioning web form (sub-project #2c-b1)

`BootMode`, `BootModeSelector`, `SetupModeRequestStore`/`NvsSetupModeRequestStore`,
`GatedDigitalInput`, `ComboSetupModeTrigger` (#2c-a), `MacAddress`,
`SetupApName`, `EspDeviceIdentity`, `WebFormSubmission`,
`WebFormCommissioningAdapter`, `SetupFormRenderer`, and `CaptivePortalServer`
(#2c-b1) are all built and (mostly) native-tested but not yet referenced by
`src/esp32/main.cpp` — wiring them in (choosing the boot mode at startup,
constructing `ComboSetupModeTrigger` against the real T1/T2
`MatrixDigitalInput`s, wrapping those two stations' inputs in
`GatedDigitalInput`, opening the `CaptivePortalServer` only in
`WirelessSetup` mode) is sub-project #2c-b2. Two open design questions
#2c-b2 must explicitly resolve, not silently inherit:
- `docs/superpowers/specs/2026-08-29-esp32-wireless-setup-trigger-design.md`'s
  "Known gap for #2c-b to resolve" section — the combo-trigger's suppression
  can arrive too late to prevent the earlier-pressed button's own command.
- `docs/superpowers/specs/2026-08-30-esp32-wireless-commissioning-web-form-design.md`'s
  "Known gaps for #2c-b2 to resolve" section — the web form's copy says a
  blank turnout-channel field unconfigures it, but the code actually leaves
  the existing name untouched; and `CaptivePortalServer` opens an
  unauthenticated WiFi AP that renders the layout's existing WiFi password
  back into the page in cleartext to anyone who joins.
